...
public:
    int numSubarrayProductLessThanK(vector<int>& nums, int k) {
        ...
        while(end<n){
            ...
            // We must write the while condition above the if condition because after sliding the window from 
            // the front till product<k, we still need to calculate the subarrays for that subarray.
