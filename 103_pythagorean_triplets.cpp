...
Ex1:   [1,3,2,5,4]
  Output: true
  Explanation: 3,4,5 are satifying the condition
...
Solution: 1. By Brute Force it will take you O(n3) time using 3 for loops.
  2. We can decrease the complexity of the code by using a hash map
    ...
    - Now the problem has reduced to "Given an array of numbers, find if two distinct numbers add up to a 
  ...
  NOTE: A tricky situation is example 2 where the update array after squaring would be [0,1,4]. 
    When i=0, j=1, 1+0=1 would be present in the map, so we need to check whether the sum of arr[i]+arr[j] is 
    ...
    This is a one-off case (0,1,1) since usually all pythogorean triplets are 3 distinct numbers.
